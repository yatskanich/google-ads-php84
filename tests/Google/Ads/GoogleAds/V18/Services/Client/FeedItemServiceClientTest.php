...
use Google\Ads\GoogleAds\V18\Services\MutateFeedItemsResponse;
 ...
 * @group gapic
 */
class FeedItemServiceClientTest extends GeneratedTest
{
    ...
    public function mutateFeedItemsTest()
    {
        ...
        $response = $gapicClient->mutateFeedItems($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v18.services.FeedItemService/MutateFeedItems', $actualFuncCall);
    ...
    public function mutateFeedItemsExceptionTest()
    {
        ...
        $request = new MutateFeedItemsRequest()
            ...
            ->setOperations($operations);
        try {
            $gapicClient->mutateFeedItems($request);
    ...
    /** @test */
    public function mutateFeedItemsAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $request = new MutateFeedItemsRequest()
            ->setCustomerId($customerId)
            ->setOperations($operations);
        $response = $gapicClient->mutateFeedItemsAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
