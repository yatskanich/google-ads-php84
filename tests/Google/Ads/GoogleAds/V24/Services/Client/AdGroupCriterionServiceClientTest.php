...
class AdGroupCriterionServiceClientTest extends GeneratedTest
{
    ...
    public function mutateAdGroupCriteriaTest()
    {
        ...
        $response = $gapicClient->mutateAdGroupCriteria($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v24.services.AdGroupCriterionService/MutateAdGroupCriteria', $actualFuncCall);
    ...
    public function mutateAdGroupCriteriaExceptionTest()
    {
        ...
        $request = new MutateAdGroupCriteriaRequest()
            ...
            ->setOperations($operations);
        try {
            $gapicClient->mutateAdGroupCriteria($request);
    ...
    /** @test */
    public function mutateAdGroupCriteriaAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $request = new MutateAdGroupCriteriaRequest()
            ->setCustomerId($customerId)
            ->setOperations($operations);
        $response = $gapicClient->mutateAdGroupCriteriaAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
